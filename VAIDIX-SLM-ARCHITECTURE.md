...
**Implementation:** One Python microservice exposing `POST /sanitize` that accepts text and returns redacted text + entity count. Called from the Next.js API layer. ~50ms latency, runs on CPU. Reference: [Microsoft Presidio analyzer + anonymizer](https://github.com/microsoft/presidio).
