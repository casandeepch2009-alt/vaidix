...
| **W6.13 — Prompt-loader infra + Call-room UX (out-of-plan, foundational)** | done | 1 dev | **Closes a foundational drift problem + 3 visible call-room UX gaps in the same sprint.** **Problem 1 — prompts hardcoded inline:** every LLM-calling service had its system prompt as a string literal in `.ts`, drifting from the canonical markdown design docs in `C:\Users\Sandeep\Desktop\New folder (3)\`. Editing a prompt meant a TS edit + redeploy. Three services (`hook-generator-service.ts`, `promo-service.ts`, `cluster-questions.ts`) bypassed the central AI router entirely. **Schema:** unchanged. **New module:** `src/server/prompts/` (single source of truth) — `_base/` holds 27 templated `.md` prompts (24 from Doc5 + 3 operational: `op-cluster-questions`, `op-promo-copy`, `op-hook-generator`); `_domains/` holds typed domain configs (currently `ophthalmology.ts`, registered in `_domains/index.ts`); `loader.ts` parses each `.md` (`H1` title, `**Target model:**` line, `## Prompt` section's fenced code block — supports both 3- and 4-backtick outer fences so prompts can contain inner ```json``` blocks naturally), interpolates `{{PLACEHOLDER}}` values from the active domain, caches per (id, domain). `interpolate.ts` throws `PromptInterpolationError` if a `.md` references a placeholder the domain doesn't provide — catches drift at load time, not runtime. `types.ts` defines `DomainConfig` (40+ fields) + `LoadedPrompt` + `PromptMetadata`. **Adding a new domain (e.g. cardiology):** one new file `_domains/cardiology.ts` + register in `_domains/index.ts` — all 27 prompts work for the new domain with zero prompt edits. **Loader features:** in-memory cache (cache hit on second `loadPrompt(id)` call), `clearPromptCache()` for tests/dev hot-reload, `listPromptIds()` for diagnostic tooling, target-model parsing (opus/sonnet/gemini/auto), token-budget parsing (handles ranges like "~3–5k" → picks upper bound 5000), `expectsJson` heuristic. **3 bypass services refactored** to call `loadPrompt('op-...')` instead of inline strings — central AI router + Gemini-direct paths preserved (no behaviour change, just sourced-from-file). **Tests:** `scripts/smoke-prompt-loader.ts` (`npm run smoke:prompts`) loads every prompt × every domain, fails CI if any prompt declares a placeholder the domain doesn't satisfy — 27/27 ok today; `scripts/e2e-prompt-loader-integration.ts` (`npm run e2e:prompts`) — 13 deterministic checks (no real LLM calls): canonical phrases present in each `.md`, inline strings GONE from the 3 refactored `.ts` files, all 3 import `loadPrompt`, placeholder substitution leaves no `{{...}}` in output, `clusterPreQuestions([])` short-circuits without calling Gemini, loader cache returns same instance on second call. Both registered in `package.json`. **Problem 2 — call-room UX gaps:** the user reported (a) 2-person call shows blank tiles when cameras off (no Teams-style avatar circles); (b) hand-raise has visual toast but no audible chime for remote raises; (c) spotlight feature presumed broken — actually shipped, just verified. **Avatar end-to-end:** new `<ParticipantAvatarCircle>` (`src/components/classroom/participant-avatar-circle.tsx`) — colored circle with photo OR deterministic initials (FNV-1a hash → 8-color palette so same identity always maps to same color, two adjacent participants rarely collide), centered over LiveKit's grey placeholder via `absolute inset-0 z-10`, `aspect-square w-[40%]` for grid tiles + `w-[55%]` for carousel thumbs. Honorific stripping in `deriveInitials()` ("Dr. Priya Sharma" → "PS" not "DP"). `<SpotlightTile>` extended to detect camera-off (`!publication \|\| publication.isMuted \|\| !publication.track` for Camera source) and overlay the avatar; spotlight star button now has `data-testid="spotlight-toggle"` + `data-spotlit` attribute for E2E assertion. Carousel layout (when screen share is focused) replaced bare `<ParticipantTile />` with `<SpotlightTile hideSpotlight avatarSize="thumb">` so screen-share mode also shows avatars. **Avatar URL plumbed DB → JWT → tile:** `mintLiveKitToken` already accepted `metadata`; `dbUser.avatarUrl` now fetched in `/api/classroom/sessions/[id]/token` (both privileged + viewer paths) AND in `mintBreakoutToken` (`breakout-service.ts`) → injected into JWT metadata → `participant.metadata` JSON parsed in `<SpotlightTile>` and passed to the avatar component. `User.avatarUrl=null` falls through to initials cleanly. **Hand-raise sound:** new `playHandRaise()` chime in `notification-sounds.tsx` — A5→C6 (880→1046 Hz, 70ms gap, 180ms total) above the join/leave register so it cuts through chatter as an attention call. Reuses the existing lazy `AudioContext`, `tone()` helper, and `STORAGE_KEY='vaidix.notificationSounds'` mute toggle (one localStorage pref silences ALL room chimes — Teams-equivalent UX). `hand-raise-notifications.tsx` calls it inside `handleMetadata` only when `!isSelf` (matches Teams: your own raise click already shows the toast immediately, no need to chime at yourself). **Spotlight verification:** `data-testid` + `data-spotlit` attributes added so the existing `SpotlightTile` is now Playwright-assertable. **Playwright UI:** new `tests/e2e/w-call-room-ux.spec.ts` (3 tests + LiveKit-reachability skip): (1) headless Chromium has no camera → faculty joins → `[data-testid="participant-avatar-circle"]` visible in own tile + `data-identity` attribute populated; (2) Web Audio recorder installed via `addInitScript` (patches `AudioContext.prototype.createOscillator` to count calls on `window.__audioOscCount`) → faculty raises hand → toast appears + oscillator count UNCHANGED (self-raise correctly suppresses chime); (3) host clicks spotlight star (force-click since opacity-0-until-hover is intentional) → `data-spotlit` attribute flips false→true→false. Follows the `w-guest-join.spec.ts` fixture pattern (BeforeAll seeds a TeachingSession with the W5 fixture faculty, AfterAll deletes). **TypeScript:** `npx tsc --noEmit` clean across all changes. **Files added:** `src/server/prompts/{loader.ts, interpolate.ts, types.ts, README.md}`, `src/server/prompts/_domains/{index.ts, ophthalmology.ts}`, `src/server/prompts/_base/*.md` (27 files), `src/components/classroom/participant-avatar-circle.tsx`, `scripts/{smoke-prompt-loader.ts, e2e-prompt-loader-integration.ts}`, `tests/e2e/w-call-room-ux.spec.ts`. **Files modified:** `src/server/services/pre-questions/cluster-questions.ts` (loadPrompt), `src/server/services/promo/promo-service.ts` (loadPrompt), `src/server/services/captions/hook-generator-service.ts` (loadPrompt), `src/components/classroom/spotlight-tile.tsx` (avatar overlay + data-testid + hideSpotlight prop + avatarSize prop), `src/components/classroom/notification-sounds.tsx` (`playHandRaise`), `src/components/classroom/hand-raise-notifications.tsx` (chime call on remote raises), `src/components/classroom/live-session.tsx` (carousel uses SpotlightTile thumb mode + remove unused ParticipantTile import), `src/app/api/classroom/sessions/[id]/token/route.ts` (avatarUrl in select + metadata for both privileged and viewer paths), `src/server/services/breakouts/breakout-service.ts` (avatarUrl fetch in `mintBreakoutToken`), `package.json` (`smoke:prompts` + `e2e:prompts` scripts). **Skipped intentionally:** porting Doc5 desktop folder out of `C:\Users\Sandeep\Desktop\` — the `_base/` directory in the repo is now the canonical source; the desktop folder becomes a working draft area. Phase-2 follow-ups: (a) optional sync script to copy from desktop → `_base/` if author prefers editing on desktop; (b) per-tenant domain selection (currently `VAIDIX_DEFAULT_DOMAIN` env or "ophthalmology"); (c) prompt versioning + A/B comparison harness; (d) `<ParticipantStrip>` and other secondary tile renderers also wrapped with the avatar overlay (currently grid + carousel are covered, the participant-strip + breakout-room-view tiles still use bare LiveKit `<ParticipantTile />` with the gray placeholder). | ✅ shipped (`npx tsc --noEmit` clean, `npm run smoke:prompts` 27/27 ok, `npm run e2e:prompts` 13/13 PASS, 2026-05-16) |
...
| B9 | **#14 Real-time live captions** (LiveKit Agent — shipped 2026-05-15) | reuses `SessionTranscript` (single row per `(sessionId, language)`); live segments fan out via Redis pub/sub `caption:<sessionId>` | bearer-authed `POST /api/classroom/sessions/[id]/live-captions/ingest` (single write path); SSE `GET /api/classroom/sessions/[id]/live-captions` (read fan-out); `POST .../captions/translate` (per-listener Gemini) | `LiveCaptionsOverlay` + `CaptionControls` in `live-session.tsx` top bar; `data-testid="live-captions-overlay"` for e2e |

**Live captions architecture (B9 — landed 2026-05-15, replaces browser-side Phase 1)**

```
┌──────────────────────────────────────────────────────────────────────────┐
│ LiveKit room: session-<sessionId>                                        │
│  ┌──────────────┐   ┌──────────────┐   ┌──────────────────────────┐      │
│  │ Host browser │   │ Resident A   │   │ vaidix-captions-agent    │      │
│  │ (mic / cam)  │   │ (mic / cam)  │   │ (Python, HIDDEN)         │      │
│  └──────┬───────┘   └──────┬───────┘   └────────────┬─────────────┘      │
│         │ audio            │ audio                  │ subscribes to all  │
│         └──────────────────┴────────────────────────┤ remote audio       │
│                                                     ▼                    │
│                                          Deepgram streaming STT          │
│                                          (1 WS per UNMUTED speaker)      │
│                                                     │                    │
│                                                     ▼                    │
│             POST /api/.../live-captions/ingest  (Bearer secret)          │
│                                                     │                    │
│                    ┌────────────────────────────────┴───────────┐        │
│                    ▼                                            ▼        │
│           Redis pub/sub `caption:<id>`            SessionTranscript      │
│                    │                              (durable, PDF source)  │
│                    ▼                                                     │
│      SSE GET /live-captions ── fan-out ── every viewer with CC=ON        │
└──────────────────────────────────────────────────────────────────────────┘
```

- One **hidden** participant (LiveKit `agent` kind = invisible) per LIVE room.
- One Deepgram stream **per unmuted speaker**, not per viewer. 100 viewers + 2 speakers = 2 ASR streams.
- Speaker attribution = LiveKit participant identity. No diarization heuristics.
- All reads (overlay SSE, PDF export, AI hook generator, post-session pack) are unchanged from the previous Phase-1 wiring — only the writer moved out of the browser.

**Required prod env (or captions are 503 even with the agent deployed):**

| Var | Used by | How to generate |
|---|---|---|
| `DEEPGRAM_API_KEY` | agent | Deepgram dashboard. Production logs on 2026-05-15 showed this missing → host CC also broken in prod. |
| `LIVE_CAPTIONS_INGEST_SECRET` | app + agent (shared) | `openssl rand -hex 32`. Set on **both** the `app` and `vaidix-captions-agent` containers; identical value. |
| `VAIDIX_INGEST_URL` | agent | `http://app:3000` inside the prod compose network; `http://host.docker.internal:3000` on Docker Desktop with native dev server. |

**Deploy step (new):**

```
docker compose -f docker-compose.prod.yml --env-file .env up -d --build vaidix-captions-agent
```

The agent registers itself with the LiveKit server on startup and is auto-dispatched to every new room. No app code change is needed to activate it once the env vars + container are in place.

**Files (B9 surface, post-2026-05-15):**

- App: `src/app/api/classroom/sessions/[id]/live-captions/{ingest,route}.ts`, `.../captions/{translate,transcript,transcript/export-pdf}/route.ts`, `src/components/engagement/live-captions-overlay.tsx`, `src/server/services/captions/{captions-pubsub,transcript-service,hook-generator-service}.ts`.
- Agent: `vaidix-agent/{main.py,requirements.txt,Dockerfile}` (~150 LOC total, one Python process).
- Deleted: `src/components/classroom/captions-producer.tsx`, `.../captions/{captions-token,publish}/route.ts` (browser-side single-speaker Phase 1).
- Tests: `scripts/e2e-w7-captions.ts` (API path), `tests/e2e/w8-cc-pdf-recording.spec.ts` (UI path inc. multi-speaker overlay assertion).
...
| C6 | Faculty document library page | reuses `Document`, `DocumentTag` (exists) | `GET /api/documents` | `/teacher/documents` |
...
| 2 | DOPS Assessment | `DopsAssessment` (exists) | `/api/teacher/dops`, `/api/teacher/dops/[id]` |
| 3 | Mini-CEX Assessment | `MiniCexAssessment` (exists) | `/api/teacher/mini-cex` |
...
- [ ] `/teacher/cohort` "scoring lands W8" placeholder is replaced with real 3H + Oslerian averages
- [ ] `/hod/competency-map` heatmap renders real entrustment levels from `EpaRecord`
...
| v1.5 | 2026-04-25 | **W6.5 polish sprint logged + §10b/§10c "pre-W8/W9 state" sections added so the W7+W8+W9 teams don't re-build what already shipped.** Status table gets a new **W6.5** row between W6 and W7 covering: session-driven `RoleProvider` replacing the demo-mode hardcoded resident; `EXTERNAL_LEARNER` role plumbed through nav + dashboard; `/admin/users` and `/admin/institution` switched from mock JSON to real DB (W1 admin APIs) with edit-role / deactivate / reactivate modals; `/teacher/learners` + `/teacher/cohort` now real DB lists with cohort filter, sessions-joined and cases-completed counts; `/hod/competency-map` set to a Week-8 banner card; engagement layer for **Pearls** (`PearlLike` heart + generic `Bookmark` + Web-Share/clipboard + "Saved only" filter) and **Recordings** (`Bookmark` + W5 `RecordingShare` token-mint modal with TTL/optional bcrypt password); **Discussion forum on saved videos** = new `qa-service.answerQuestion` + `PATCH /api/classroom/sessions/[id]/qa/[qaId]/answer` (FACULTY/PD/ADMIN/host, audited via new `qa.question.answered` / `qa.answer.cleared` events); `QaSidebar` rewritten with prominent green "Answered by Dr. X" block above the question; `/profile` + `/profile/bookmarks` (pearls + recordings sections) + Saved-items card on profile; `signOut()` properly wired through next-auth; root-redirect chain fixed (no more `?callbackUrl=http%3A%2F%2F...`); dead `/admin/roles` + `/admin/image-library` sidebar entries removed; demo seed (`prisma/seed.demo.ts`) for 4 demo users + 5 sessions wired via `npm run db:seed:demo`. **Schema unchanged** — additive UI on existing W0-locked tables. **§10b Week 8** now opens with a "⚠️ Pre-W8 state" table calling out 4 already-built pages (`/teacher/learners`, `/teacher/cohort`, `/hod/competency-map`, `/admin/users`) with extend-don't-replace guidance, plus a pointer to `engagement-service` and the new `qa.answerQuestion` for Faculty-dashboard "questions awaiting your answer" widget. **§10c Week 9** opens with a "Pre-W9 state" table noting that the Pearls engagement layer is already shipped and W9 only swaps the in-memory JSON filter for `db.pearl.findMany`. See **[E2E-FRONTEND-AUDIT.md](E2E-FRONTEND-AUDIT.md)** for the full role-by-role click-walk. Build clean: `npx tsc --noEmit` exit 0, `npm run build` `✓ Compiled successfully`. |
| v2.7 | 2026-05-16 | **Deploy pipeline — turnserver.conf + livekit.prod.yaml templated from .env; render-configs.sh refuses placeholders; scripts/deploy.sh is the single command for routine code deploy.** Closes the v2.4 incident class that resurfaced today as the "guest's name keeps refreshing" / "video call not able to join" failure — `turnserver.conf` and `livekit.prod.yaml` were tracked in git with `CHANGE_ME_STRONG_TURN_PASSWORD` literal placeholders, with an operator-runbook note saying "remember to sed this on the box". Operators didn't, the placeholder shipped to prod, and LiveKit advertised only host ICE candidates (the Docker-internal 172.18.x.x address) — clients on mobile data / corporate Wi-Fi had nowhere to relay to. The same EC2 box also still had the pre-v2.6 deck-forge container running because operators reached for the CONTAINER name `vaidix-app` when the compose SERVICE name is `app` — `docker compose ... build --no-cache vaidix-app` silently no-op'd, leaving the v2.6 fix on disk but never in the running image. **Templates** (new, tracked): `turnserver.conf.tpl` and `livekit.prod.yaml.tpl`, both with `${TURN_SHARED_SECRET}` / `${COTURN_EXTERNAL_IP}` / `${COTURN_REALM}` substitutions. The SAME `${TURN_SHARED_SECRET}` env value renders into BOTH files — drift between coturn auth and LiveKit's `turn_servers.credential` becomes structurally impossible. `livekit.prod.yaml.tpl` also threads `${LIVEKIT_API_KEY}` and `${LIVEKIT_API_SECRET}` (was: hardcoded `devkey: secret_change_me_in_week_0_day_5_32chars_min`). **Rendered files removed from git tracking** (matching the existing `egress.yaml` pattern): `turnserver.conf` and `livekit.prod.yaml` are now `.gitignore`d deploy artifacts. **`scripts/render-configs.sh`** extended to render all three templates and to validate up-front: refuses on empty value, refuses if any required env contains substring `CHANGE_ME`, sanity-checks `LIVEKIT_INTERNAL_WS_URL` starts with `ws://`/`wss://`, sanity-checks `COTURN_EXTERNAL_IP` is a bare IPv4 (catches the common operator confusion of putting the DNS name there). `turnserver.conf` chmod tightened to 640 (carries the TURN secret). **`scripts/deploy.sh`** (new) is the single command operators run on EC2: stashes nginx edits → pulls → renders configs → applies migrations → rebuilds `app` ONLY when `src/`/`prisma/`/`package.json`/`next.config.ts`/`Dockerfile` changed → recreates `livekit + coturn` ONLY when their templates changed → recreates `livekit-egress` ONLY when egress template changed → reloads `nginx` ONLY when its sites changed → health-checks. Uses CORRECT compose service names (`app`, `livekit`, `coturn`, `livekit-egress`, `nginx`) — not the container names that caused the v2.6 silent no-op. **Env examples**: `.env.production.example` adds the three TURN vars with `CHANGE_ME_*` defaults + generate-command + guidance ("DNS goes in REALM, bare IPv4 in EXTERNAL_IP"). `.env.example` (dev) adds dummy values so `render-configs.sh` works locally for non-prod testing. **`DEPLOYMENT.md` §4 (Routine code deploy)** rewritten — "On EC2: `./scripts/deploy.sh`. That's it." Old multi-step manual sequence kept as appendix for context. New "Rotating secrets" subsection documents the idempotent `upsert_env` helper for `.env` mutations without disturbing other entries. **Smoke** (`scripts/smoke-render-configs.sh`, 20/20 PASS): clean env renders all three configs + every env value substitutes correctly + ZERO `CHANGE_ME` placeholders ever appear in rendered output (prod-safety invariant) + render REFUSES when TURN_SHARED_SECRET contains CHANGE_ME + REFUSES on empty COTURN_EXTERNAL_IP + REFUSES on DNS-name-in-IP-field + REFUSES on http:// scheme for WS URL + REFUSES on missing .env. **User-perspective e2e (frontend angle)**: cannot be run locally — requires a real EC2 box + real mobile-network browser to test the symmetric-NAT path that needs TURN relay. Validation flow post-deploy: (1) load session URL on phone using mobile data (not Wi-Fi), (2) host admits from waiting room, (3) audio + video stream both ways without the "Reconnecting…" overlay flashing every 15s. Pre-v2.7: this consistently failed. Post-v2.7 with correctly rotated `TURN_SHARED_SECRET` + open AWS Security Group ports: expected to succeed. Tests/e2e/w-guest-resilience.spec.ts (v2.4) already covers the reconnect overlay UI; the wire-level TURN advertisement is verified by `docker logs vaidix-livekit \| grep turn` after deploy. **Out of scope** (deferred): `tests/e2e/w-deploy-smoke.spec.ts` Playwright test that mocks a TURN-allocation failure and asserts the "Connection lost — Rejoin" panel renders — Phase 3 backlog. **Build status**: `npx tsc --noEmit` exit 0; `bash scripts/smoke-render-configs.sh` 20/20 PASS. **Deploy on EC2 (first time after v2.7)**: (a) `git pull`, (b) edit `.env` to add `TURN_SHARED_SECRET`/`COTURN_EXTERNAL_IP`/`COTURN_REALM` via the upsert_env helper in DEPLOYMENT.md §4, (c) `./scripts/deploy.sh`. Subsequent deploys: just `./scripts/deploy.sh`. |
| v2.6 | 2026-05-16 | **Wizard Forge — wired through the central prompt loader; HARD ENHANCE_EXISTING contract; coverage > strict count; per-slide imageBrief drives content-aware Gemini Image generation.** Audit triggered by faculty report ("Fellow-Level-Clinical-Lecture.pptx" output had none of DR AVI PPT's endophthalmitis topics, placeholders instead of images). Cross-referenced with VAIDIX_MASTER_REFERENCE.md from the master prompt folder: of 24 Doc5-module prompts in `src/server/prompts/_base/`, ZERO were wired into production code — only 3 short `op-*` operational siblings (`op-hook-generator` / `op-promo-copy` / `op-cluster-questions`) called `loadPrompt()`. The deck-forge wizard carried hardcoded TS string prompts that drifted from Doc5 §4.1.1 (Presentation Coach Super Prompt) for 3 weeks. **Phase 1 — sync** (`feat/prompt-loader-wire-deck-forge` first commit): overwrote all 24 in-repo `_base/` prompts with the tightened versions from the master folder (`4.1.1` and `4.1.3` had drifted), wrapped 5 spec-form prompts (`4.1.3` / `5.1.1` / `5.1.3` / `flashcards` / `infographic`) in the loader's required `## Prompt` + 4-backtick fence — content preserved verbatim, only adding the loader markers. New `scripts/smoke-prompts.ts` walks every prompt × every registered domain at build time, fails on `PromptNotFoundError` / `PromptParseError` / `PromptInterpolationError` — the CI gate the README §"When the loader fails" promised but never shipped (27/27 cases pass currently). **Phase 2 — wire deck-forge** (second commit): three new operational prompts authored as siblings to Doc5 §4.1.1 (Doc5 spec stays as the canonical Super Prompt for future standalone use; the op-* variants emit the strict JSON the wizard's normalize() expects): `_base/op-deck-extract.md` (Gemini multimodal source ingest), `_base/op-deck-draft.md` (Opus deck author with HARD ENHANCE contract — see below), `_base/op-deck-image-prompt.md` (Gemini Flash image-prompt writer). All three are `{{DOMAIN_*}}`-templated so cardiology / dentistry adoption is a single `_domains/<name>.ts` file. **`wizard-forge-service.ts`**: deleted both hardcoded TS prompt constants (~95 lines combined), `extractFromSources` + `draftFromExtraction` now call `loadPrompt('op-deck-extract')` / `loadPrompt('op-deck-draft')` at request time. **`router.ts`**: `aiGenerateImagePrompt` deleted its hardcoded constant, loads `op-deck-image-prompt` (dynamic import to avoid the prompts module pulling the AI router into a cycle). **HARD ENHANCE_EXISTING contract (the DR AVI regression root cause):** previous prompt said "Keep original titles when possible" — too soft, model silently invented "Fellow-Level Clinical Session". New `op-deck-draft.md` has five HARD rules: (1) TITLES ARE FROZEN — output title MUST equal primaryDeckOutline entry verbatim; rename proposals emit as `initialSuggestion` instead, (2) ORDER PRESERVED with insert/merge allowances, (3) CITATIONS MANDATORY on every kept slide, (4) CONTENT may be upgraded but not invented, (5) COVERAGE OF ALL TOPICS — dropping a primaryDeckOutline entry is a contract failure; supplementary PDF topics MUST also be covered. **Coverage > count:** original prompt capped 60-min decks at 14-22 slides — strict cap caused under-coverage when supplementary PDFs added topics. New language: slide-count range is GUIDANCE, not a cap; ENHANCE floor is original count + supplementary coverage, often exceeds upper guidance. `MAX_SLIDES_PER_DECK` raised from 30 → 80, bullets/slide 6 → 8, speakerNotes 1000 → 1500 chars. `maxTokens` for Opus draft raised 8000 → 16000 so coverage isn't truncated. **Per-slide `imageBrief` (new field on DraftedSlide):** Opus now writes a dynamic per-slide visualization brief (≤240 chars, anatomy-specific) whenever a clinical visualization helps the teaching point — not only on IMAGE_FOCUS layouts. `aiGenerateImagePrompt`'s `ImagePromptInput` type gains `imageBrief?` + `speakerNotes?`. `op-deck-image-prompt.md` declares input precedence (imageBrief > bullets > speakerNotes) so the image-prompt writer leans on Opus's clinical seed rather than generic bullet inference. `generateSlideImages` re-defined to take the rich `normalized result.slides` directly (paired with persisted slide ids post-`createMany`) so `imageBrief` flows through; candidate set expanded from `layout === IMAGE_FOCUS` only → `layout === IMAGE_FOCUS OR imageBrief set`. **Service smoke** (`scripts/smoke-wizard-forge.ts`, HTTP-free, real Gemini + Opus when reachable): 21/21 structural assertions pass on the local dev box — PptxDocument round-trip, notes() extraction (PEARL: text on 7/7 slides), `extractPptxContent` verbatim outline + hasSpeakerNotes flag, all three op-deck-* prompts load with the HARD CONTRACT / coverage / imageBrief markers present in their text. Phase B (the actual `wizardForgeDeck()` call → Gemini → Opus → Gemini Image → S3) skips with a clear log when Gemini is unreachable (local dev with corporate TLS interception hits `UNABLE_TO_VERIFY_LEAF_SIGNATURE`); production EC2 reaches Google APIs cleanly. **What's still on hardcoded TS prompts (Phase 3 backlog, one feature per PR with JSON-schema reconciliation):** all 21 other Doc5 modules (4.1.2 / 4.1.3 / 4.1.4 / 4.1.5 / 5.1.x / 6.1.x), plus `deck-analyze-service.ts` and `deck-refine-service.ts`. **Build status**: `npx tsc --noEmit` exit 0, `npm run build` exit 0, `npx tsx scripts/smoke-prompts.ts` 30/30 PASS. **No schema change.** Deploy = rebuild + restart on EC2 (correct service name `app`, not `vaidix-app`). |
| v2.5 | 2026-05-16 | **Wizard Forge + Objectives Suggest — real .pptx ingestion (titles + body + Mac/Win speaker notes), generated images on output deck, Apple Keynote (.key) accepted, single shared PPTX extractor.** Root cause documented from a faculty report: uploading `DR AVI PPT.pptx` with `intent=ENHANCE_EXISTING` produced a generic "Knowledge Update for the Practicing Ophthalmologist" output deck unrelated to the endophthalmitis decision-tree in the source. Same defect surfaced silently on `/api/classroom/sessions/[id]/objectives/suggest`: PPTX uploads in the study pack were counted as `truncated` (not ingestible) and the AI returned objectives drafted from session title + description alone. Investigation found `extractFromSources` in `src/server/services/decks/wizard-forge-service.ts` checked `GEMINI_INLINE_MIMES` (pdf / png / jpeg / webp / text / md) and fell through to a `Phase 2 will swap in a real .pptx text extractor` placeholder for every .pptx mime — Gemini received only the filename + briefing, returned no `primaryDeckOutline`, and the Opus draft prompt's ENHANCE branch (`Keep the original slide order and titles when possible`) silently fell through to free-form draft. The in-tree `PptxDocument` class (`src/server/services/pptx/pptx-document.ts`) had a complete .pptx text extractor with stable slotIds but zero callers — dead code. Same dead-code pattern for `aiGenerateImage*` in `src/server/services/ai/router.ts` — defined, never invoked, so `IMAGE_FOCUS` slides always painted the `[ Image / OCT / fundus photo placeholder ]` dashed rectangle. **Schema** (`prisma/migrations/20260516000000_slide_images/migration.sql`, additive, idempotent `ADD COLUMN IF NOT EXISTS`): `Slide.imageS3Key TEXT?` + `Slide.imagePrompt TEXT?`. Both nullable — a missing image never blocks the deck. **PptxDocument** (`src/server/services/pptx/pptx-document.ts`): new `notes(slideIndex)` method walks `ppt/slides/_rels/slideN.xml.rels`, resolves the `...relationships/notesSlide` target, parses `ppt/notesSlides/notesSlideM.xml`, returns the concatenated `\n`-joined notes text. Round-trips across Mac Keynote → .pptx export and Windows PowerPoint; out-of-range slideIndex throws PptxParseError, missing/empty/malformed notes return `''` (caller treats as "no notes"). **Wizard service** (`wizard-forge-service.ts`): renamed `fetchInline` → `fetchBytes` (returns Buffer; base64 conversion moved to the inlineData call site so the same fetch helper feeds both Gemini and PptxDocument). New `PPTX_MIME_TYPES` set + `extractPptxContent(buf, label)` helper (now EXPORTED for cross-service reuse — see suggest-objectives below) that uses `PptxDocument.fromBuffer(buf).slides()` and `.notes(index)` to emit a structured `=== Slide N === TITLE: … / TEXT: … / NOTES: …` text part (200 KB/doc cap) AND a deterministic `outline` array (slideIndex + title + 300-char summary) + a `hasSpeakerNotes: boolean` flag. On `PptxParseError`, falls back to the old metadata-only placeholder so a malformed .pptx never breaks the forge. After `aiExtractFromSourceJson`, the deterministic `primaryOutline` **overwrites** `extraction.primaryDeckOutline` — Gemini drifts on slide titles and the ENHANCE prompt requires verbatim anchor. New `KEYNOTE_MIME_TYPES` set (`application/vnd.apple.keynote` + legacy `application/x-iwork-keynote-sffkey`) routes .key uploads through a clear-message metadata-only path: text isn't extractable yet (iWork iwa proto-binary archive needs a dedicated parser), prompt advises faculty to export → .pptx for full ingestion. New `generateSlideImages({jobId, requestedById})` step inserted between the slide-persist transaction and `persistDeckAsDocument`: queries back `IMAGE_FOCUS` slides, batches `aiGenerateImageForSlide({title, bullets})` calls with concurrency cap of 3 to respect Gemini Image RPS, uploads each PNG to S3 at `documents/deck-forge/<userId>/<jobId>/slide-<order>.png`, persists `imageS3Key` + `imagePrompt` back on the Slide row. Per-slide and aggregate failures are caught + logged — image richness is a nice-to-have, never a deck blocker. **Objectives suggest** (`src/server/services/objectives/suggest-service.ts`): now imports `extractPptxContent` + `PPTX_MIME_TYPES` from the deck-forge service so .pptx study-pack docs flow through the SAME extractor (single source of truth — no parallel implementation). PPTX path runs before the inlineData branch; parse failure increments `truncated` and the UI surfaces the warning. NO_MATERIAL error copy updated to list PPTX in the accepted formats. **Mime map** (`src/server/services/documents/document-service.ts`): `KIND_FROM_MIME` adds `application/vnd.apple.keynote` and `application/x-iwork-keynote-sffkey` → `DocumentKind.PPT` so Keynote uploads classify cleanly. **UI accept lists**: `wizard-client.tsx` PRIMARY_PPTX uploader now accepts `.ppt,.pptx,.key,.pdf`; `session-upload-button.tsx` study-pack uploader adds `.key` to its existing list. **Renderer** (`src/server/services/decks/deck-pptx-renderer.ts`): `SlideRow` gains `imageDataUrl: string | null`. `renderDeckPptxBuffer` pre-resolves S3 keys → data URLs in `Promise.all` before the synchronous pptxgenjs render loop (avoids serial S3 calls inside `forEach`). `renderImageFocus` paints `s.addImage({data, sizing: {type: 'contain', w: 11.5, h: 4.0}})` when `imageDataUrl` is set; falls back to the existing dashed-rectangle placeholder when null or S3 fetch fails. Legacy `deck-forge-service.ts` callers (whose slides have `imageS3Key = null`) round-trip unchanged. PDF / image / plain-text inputs go through the same path as before — `d.mimeType` (DB-stamped by `document-service.ts`) now feeds inlineData instead of S3's `ContentType` (more authoritative). Unhandled binaries (.doc, .docx, .ppt OLE) keep the metadata-only signal. **Test**: `scripts/e2e-w-pptx-enhance.ts` builds a 7-slide synthetic endophthalmitis fixture (mirrors DR AVI PPT structure) with speaker notes on every slide, uploads as PRIMARY_PPTX, posts to `/api/decks/wizard/forge` with `ENHANCE_EXISTING`, asserts (a) `extractedPearls.primaryDeckOutline` titles match fixture verbatim, (b) `extractedPearls.topics`/`keyFacts` mention ≥2 fixture-only clinical terms (`vanco`, `ceftaz`, `evs`, `ppv`, `hypopyon`, `intravitreal`, `tap`) — proves Gemini read PPTX content, not just briefing, (c) ≥3 drafted slide titles overlap fixture titles, (d) if Opus produced any IMAGE_FOCUS slide, ≥1 has `imageS3Key` + `imagePrompt` set + S3 object is `image/*` with `ContentLength > 1000`, (e) persisted .pptx round-trips through `PptxDocument` with `slideCount` matching DB. Out of scope (kept as metadata-only placeholders, documented in extractor): old .ppt OLE binary (different format — needs `unoconv` or libreoffice-headless), `.doc`/`.docx` (needs `mammoth`), Keynote `.key` text/notes parsing (needs iWork iwa parser — accepted today as metadata-only with a clear "Export to .pptx" hint). Build status: `npx tsc --noEmit` exit 0. **Run after pull**: `npx prisma generate && npx prisma migrate deploy` (or `migrate dev` locally — additive, no data loss). |
| v2.8 | 2026-05-16 | **Deploy discipline + git hooks + frontend ENHANCE e2e + smoke-all umbrella.** Closes three post-v2.7 gaps identified by the user after the fourth deploy log showed stale containers (vaidix-app Up 4 hours, vaidix-livekit Up 2 days) despite correct config renders. Root cause of the deploy gap: `scripts/deploy.sh` set `CHANGED_FILES=""` when `BEFORE_SHA == AFTER_SHA` (git already up to date) and then skipped ALL container rebuilds — the "no new commits since last deploy" branch had no awareness of container staleness. Root cause of the discipline gap: commit `ee7f905` landed directly on master (a chmod hotfix for the deploy.sh executable bit) — no branch, no review. **(1) deploy.sh — `--force` flags.** Three new optional flags parsed at the top of the script: `--force` (rebuild + recreate all affected services), `--force=app` (rebuild + recreate `app` only), `--force=livekit` (recreate `livekit` + `coturn`), `--force=egress` (recreate `livekit-egress`). The flags set `FORCE_APP` / `FORCE_LIVEKIT` / `FORCE_EGRESS` variables, each ORed into the corresponding `NEEDS_*` decision. When no force flag is given AND `BEFORE_SHA == AFTER_SHA`, a clear warning is printed: `⚠ No --force flag given. If containers are stale, re-run with: ./scripts/deploy.sh --force`. This converts the previous silent skip into an explicit operator choice — "I want to skip rebuilds" becomes something you must opt in to with the absence of flags AND acknowledge the warning. Operator fix for the 2026-05-16 incident: `./scripts/deploy.sh --force` will rebuild app, recreate livekit + coturn + livekit-egress in one command. `bash -n` syntax check: clean. **(2) Git discipline hooks.** `.githooks/pre-push`: blocks any direct push to `master` or `main`; prints a branch-and-PR instructions message; emergency bypass is `git push --no-verify` with the expectation of a Slack note. `.githooks/pre-commit`: runs `bash -n` on every `.sh` file in the staged changeset; aborts the commit if any script has a syntax error. `scripts/install-git-hooks.sh`: single command to wire both hooks into the local clone via `git config core.hooksPath .githooks` + `chmod +x`. All contributors who run install-git-hooks.sh after their next `git pull` get the guard. The hooks are tracked in the repo so they version with the code. **(3) `scripts/smoke-all.sh`.** Umbrella runner that calls `smoke-render-configs.sh` → `smoke-prompts.ts` → `smoke-wizard-forge.ts` in sequence, collects pass/fail counts, and exits 1 if any suite failed. CI calls this single script instead of managing three separate invocations. Results are displayed per-suite so a failure is immediately localised. **(4) `tests/e2e/w-pptx-enhance.spec.ts`** (4 Playwright cases, frontend-user perspective, all API calls intercepted). **Case 1 — happy path ENHANCE_EXISTING**: faculty logs in, navigates to /teacher/decks/new, picks ENHANCE_EXISTING, continues to step 2; asserts `zone-primary` visible + `zone-sources` visible + Continue DISABLED; uploads a synthetic .pptx (ZIP magic bytes, correct mimeType) via `upload-primary-input`; after intercepted upload resolves (returns FAKE_PRIMARY_DOC_ID) Continue enables; fills briefing; step 4 confirms shows ENHANCE-specific copy; forge button fires POST intercepted; asserts `intent=ENHANCE_EXISTING` + `inputs[0].role=PRIMARY_PPTX` + `inputs[0].documentId=FAKE_PRIMARY_DOC_ID`. **Case 2 — disabled gate**: ENHANCE intent selected, no file uploaded, Continue stays disabled for 500 ms — proves the required-field guard hasn't regressed. **Case 3 — supplementary source alongside PRIMARY_PPTX**: uploads .pptx to zone-primary (first upload call → FAKE_PRIMARY_DOC_ID) + PDF to zone-sources (second upload call → FAKE_SOURCE_DOC_ID); forge payload has `role=PRIMARY_PPTX` for the PPTX and `role=SOURCE` for the PDF — proves the role-routing invariant that prevents the "inventive deck" regression. **Case 4 — DRAFT_FROM_SCRATCH hides zone-primary**: picks DRAFT intent, continues; asserts `zone-primary` count=0. TypeScript: `npx tsc --noEmit` exit 0 (no errors, no output). **What the spec does NOT cover** (explicitly out of scope, covered by other tests/smokes): the actual AI pipeline (extraction + draft + image gen) — that's `scripts/e2e-w-pptx-enhance.ts`; the TURN relay path — requires a real mobile network, not testable in Playwright; container-age detection (not implemented — `--force` is the operator tool instead). **No schema change. Deploy**: `git pull && ./scripts/install-git-hooks.sh` on each dev machine; EC2 gets the hooks on next `git pull`; to clear the stale containers run `./scripts/deploy.sh --force` once. |
| v2.4 | 2026-05-15 | **Live-call resilience — guest reconnect overlay + waiting-room chime + egress storm + TURN wiring.** Fixes three production bugs surfaced by LVPEI faculty on the 2026-05-15 grand-rounds session. **(1) "Guest's name keeps refreshing" / "looks like they logged out and back in."** Root cause: LiveKit was only advertising `host`-type ICE candidates (verified from `livekit.transport.pion.ice` logs: `Failed to ping without candidate pairs. Connection is not possible yet.` followed by `state=failed requestsSent=8 responsesReceived=0`). Mobile-hotspot guests behind symmetric NAT had no way through, the PeerConnection timed out, and `onDisconnected` in `guest-prejoin.tsx:163-168` recycled state straight to NAME — every blip walked the user through the lobby again and joined LiveKit with a fresh `participant_id`, which the host's sidebar rendered as "old name disappeared, new name appeared." Server fix: `livekit.prod.yaml` now declares `rtc.turn_servers` (UDP + TCP entries) pointing at the existing vaidix-coturn container; `turnserver.conf` switched from `use-auth-secret` to static `lt-cred-mech` because LiveKit's external `turn_servers` config passes credentials to clients verbatim and has no path to mint time-limited tokens against an external coturn. Operator must mirror the `CHANGE_ME_STRONG_TURN_PASSWORD` placeholder in both files and set `external-ip` to the EC2 public IP before restart. Client fix: `guest-prejoin.tsx` `LobbyState.JOINED` gained a `phase: 'live' | 'reconnecting' | 'lost'` discriminant; `onDisconnected(reason)` only recycles to NAME when the reason is **terminal** (CLIENT_INITIATED / PARTICIPANT_REMOVED / ROOM_DELETED / USER_REJECTED / DUPLICATE_IDENTITY), anything else flips to `reconnecting` and a new effect polls GET `/api/classroom/sessions/[id]/guest` every 1.5s for up to 30 s to re-mint a JWT — `<GuestLiveRoom key={token}>` then re-mounts cleanly with the fresh token. A translucent "Reconnecting…" overlay with `role="status" aria-live="polite"` covers the room during the swap; if the budget runs out we render a "Connection lost — Rejoin" panel instead of a frozen black screen. **(2) "No sound when someone's in the waiting room."** `notification-sounds.tsx` only hooked `RoomEvent.ParticipantConnected/Disconnected`, which fires for *LiveKit-room* arrivals — guests waiting for admission never reached the room and so never chimed. Added `playWaitingRoomKnock()` (3-note E5→A5→E5 arpeggio, distinct from the join chime so moderators can tell "guest in waiting room" from "participant just joined the call") gated by the same `localStorage` mute key — one toggle silences everything. `participant-sidebar.tsx` polls admissions every 5 s; a `useRef<Set<string>>` of previously-seen admission ids fires the chime only on a **new** id arriving — admit/deny round-trips that shrink the list don't mis-fire. A `primedRef` skips the first poll so a moderator opening the page mid-call doesn't hear a chime for guests already in the queue. **(3) Egress storm — `egress_aborted: Start signal not received` every ~20 s.** Root cause: `egress.yaml.tpl` carried `ws_url: ws://192.168.1.7:7880` literally — a developer LAN IP that shipped to prod, where it points at nothing. The egress Chrome bot tried to reach it, timed out, aborted; every subsequent `track_published` webhook re-triggered the loop. Fix: `egress.yaml.tpl` now reads `ws_url: ${LIVEKIT_INTERNAL_WS_URL}` and `scripts/render-configs.sh` validates the env var is set AND starts with `ws://` or `wss://` before rendering. Default `LIVEKIT_INTERNAL_WS_URL=ws://livekit:7880` documented in both `.env.example` and `.env.production.example` — Docker service-name resolution on `vaidix-net` is the right call inside the cluster (skips TLS termination, lower latency). **Tests**: `scripts/e2e-w-guest-resilience.ts` covers the API contract for the reconnect flow — POST/GET poll → re-call GET after the first JOINED returns a fresh token while keeping the SAME admission row, terminal-vs-transient invariants on the admission state machine, and that re-mint is idempotent (same admission id, distinct JWT). `tests/e2e/w-guest-resilience.spec.ts` drives the rendered UI: anonymous join → host admits → assertion of the live room → simulated transient disconnect via `LiveKitRoom` ref shows the "Reconnecting…" overlay (NOT the lobby), then host triggers admission for a SECOND guest while the first is mid-call → host browser hears the waiting-room chime (asserted by intercepting `AudioContext.prototype.createOscillator` calls). **Operator runbook (deploy this on the EC2 box)**: (a) set `LIVEKIT_INTERNAL_WS_URL=ws://livekit:7880` in `/var/lib/vaidix/.env`; (b) replace `CHANGE_ME_STRONG_TURN_PASSWORD` in both `turnserver.conf` and `livekit.prod.yaml` with a strong shared secret; (c) set `external-ip=<EC2 public IP>` and `realm=turn.<your domain>` in `turnserver.conf`; (d) open AWS security group inbound UDP 3478 + TCP 3478 + UDP 49152-65535; (e) `./scripts/render-configs.sh && docker compose -f docker-compose.prod.yml up -d --force-recreate livekit livekit-egress coturn` to roll the three affected containers. **Schema unchanged.** Build clean: `npx tsc --noEmit` exit 0. |
| v2.3 | 2026-05-14 | **Anonymous guest join — Teams-parity for `openToAll` sessions.** Anyone with a `/classroom/[id]` link can now join an `openToAll = true` APPROVED session without a Vaidix account; they type a display name, the host admits from the in-call waiting room, and they land in the LiveKit room as `PARTICIPANT` (or `VIEWER` in webinar mode). Closed sessions still redirect to `/login`. **Schema** (additive, no data loss): `SessionAdmission.userId` made nullable; new nullable `guestKey` column carrying a 24-byte base64url token mirrored in an HttpOnly cookie `vdx_guest_<sessionId>` on the guest's browser (6 h TTL). New partial-unique `(sessionId, guestKey)` alongside the existing `(sessionId, userId)` — Postgres NULL-not-equal semantics let both coexist on the same table. **Service** (`src/server/services/admission-service.ts`): `requestAdmission` overloaded to accept either `{userId}` or `{guestKey, displayName}`; `getGuestAdmissionStatus` added; `admit` skips the `SessionInvite` upsert when `adm.userId` is null (the ADMITTED row itself is the token-gate proof); `deny` + audit summaries handle the null-userId case. **Public API** (`src/app/api/classroom/sessions/[id]/guest/route.ts`): POST `{name}` registers/refreshes the admission and sets the cookie; GET polls and, on ADMITTED, mints a LiveKit JWT with identity `guest_<admissionId>` and the typed name. 403 `OPEN_NOT_PERMITTED` shields closed sessions from stale cookies. **Route restructure**: `/classroom/[id]` root page moved out of `(platform)` into a new `(call)` route group whose layout does no auth check — the page itself branches authed → existing `<LiveSession>`, anonymous + openToAll → new `<GuestPrejoin>` (name → waiting → minimal LiveKitRoom with mic/cam/screen + leave), anonymous + closed → `redirect('/login?next=…')`. `/classroom/[id]/edit`, `/study`, `/pre-questions`, `/recording` stay under `(platform)` so the auth-required subroutes are unchanged. **Host UI**: `participant-sidebar.tsx` renders guest rows with a "Guest" badge and "Anonymous guest — joined via link" subtitle when `p.user` is null; `PendingAdmission.user` made nullable in `video-room-client.tsx`. **Tests**: `scripts/e2e-w-guest-join.ts` covers the API contract end-to-end — POST/idempotency, closed-session 403, host-side `user: null` in pending list, admit-without-SessionInvite invariant, LiveKit JWT identity/name claims after admit, deny + re-request reset. `tests/e2e/w-guest-join.spec.ts` drives the browser surface: anonymous visit renders the name card (not /login), closed session still redirects to /login, full happy-path admit transition (LiveKit-gated, skipped when the local LiveKit container isn't reachable). `tests/e2e/w2-live-prejoin.spec.ts` "unauthenticated visit" assertion updated to accept either the guest prompt or the /login redirect, since the fixture session is `openToAll`. **Build prerequisites**: schema change requires `npx prisma generate && npx prisma migrate dev --name guest_admission_nullable_user` — run after stopping `next dev` so the Windows DLL lock releases. **No vendor names client-visible**: LiveKit identity prefix `guest_` and the JSON payload don't surface anywhere in the UI (per `feedback_no_model_names_in_ui.md`). **Decisions captured**: gating = `openToAll` flag (rejected share-token gating as too implicit); guest role = PARTICIPANT (mic/cam/screen-share allowed, no admin grants). |

---

*Document Version: 2.8*
*Status: W0–W6 shipped (W4 89/89 e2e + reviewer fixes; W5 e2e + Playwright PASS; W6 P1 26/26 + W6 P2 17/17 PASS, all on 2026-04-27). W6.5 polish sprint shipped 2026-04-25; W6.6 admin-invitations polish + W6.7 cohort/session-invite UI completion + v1.8 cohort CRUD + orphan-FK hardening + calendar redesign shipped 2026-04-26. v1.9 org-mapping (Cohort↔Faculty mentor, Faculty↔PD) shipped 2026-05-01. v2.0 session pre-flight model (window-gated LIVE/recording/captions) shipped 2026-05-10. v2.1 bulk-invitations Phase 1 (admin Excel upload, /admin/invitations/bulk + POST /api/admin/invitations/bulk) shipped 2026-05-10. v2.1.1 mobile-conflict guard: createInvitation now throws MOBILE_EXISTS / MOBILE_INVITE_EXISTS; bulk preview detects in-batch mobile duplicates as warnings; both routes surface human-readable messages shipped 2026-05-11. v2.3 anonymous-guest join (Teams parity for openToAll sessions) shipped 2026-05-14 — schema + service + public /guest API + (call) route group + GuestPrejoin UI + host-side guest badge + API/UI e2e; needs `npx prisma generate && prisma migrate dev` after stopping next dev. v2.4 live-call resilience (guest reconnect overlay, waiting-room chime, egress storm fix, TURN wiring) shipped 2026-05-15 — three live-call bugs surfaced by LVPEI grand rounds; requires operator deploy steps documented in the v2.4 change-log row. v2.5 wizard-forge real .pptx ingestion + IMAGE_FOCUS image generation shipped 2026-05-16 — closes the "enhance produces unrelated deck" bug surfaced by DR AVI PPT regression; needs `npx prisma generate && npx prisma migrate deploy` after pull. v2.6 wired deck-forge through the central prompt loader + HARD ENHANCE_EXISTING contract + coverage > strict count + per-slide imageBrief for content-aware Gemini Image generation shipped 2026-05-16 — closes the "Fellow-Level-Clinical-Lecture.pptx invented new titles + placeholder images" regression; 27/27 prompt-loader smoke + 21/21 wizard-forge structural smoke PASS locally; no schema change, deploy is rebuild + restart only (service name `app`, not `vaidix-app`). v2.7 turn-config templates + scripts/deploy.sh + scripts/smoke-render-configs.sh shipped 2026-05-16 — closes the v2.4 TURN-placeholder + v2.6 wrong-service-name incident class; `turnserver.conf` + `livekit.prod.yaml` removed from git tracking, rendered from `.env` via render-configs.sh which refuses placeholders; deploy is `./scripts/deploy.sh`; 20/20 render-configs smoke PASS locally; first run needs operator to upsert TURN_SHARED_SECRET / COTURN_EXTERNAL_IP / COTURN_REALM in `.env` per DEPLOYMENT.md §4. v2.8 deploy discipline + git hooks + frontend ENHANCE e2e + smoke-all umbrella shipped 2026-05-16 — fixes deploy.sh stale-container blind spot (--force flags), adds pre-push/pre-commit hooks blocking direct-to-master + bash-n syntax checks, adds smoke-all.sh umbrella runner, adds 4-case Playwright spec w-pptx-enhance.spec.ts proving the ENHANCE wizard's PRIMARY_PPTX upload path and role-routing invariant; `npx tsc --noEmit` exit 0, all bash -n clean; no schema change; on EC2 run ./scripts/deploy.sh --force to clear stale containers; run ./scripts/install-git-hooks.sh on each dev clone.*
