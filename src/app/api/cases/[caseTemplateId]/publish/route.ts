// POST /api/cases/[caseTemplateId]/publish — promote draft to program bank.
...
import { publishCaseTemplate, CaseForgeError } from '@/server/services/cases/case-forge-service';
...
export async function POST(req: Request, ctx: { params: Promise<{ caseTemplateId: string }> }) {
  ...
  try {
    await publishCaseTemplate(caseTemplateId, auth.user.id);
    await audit({
      actorId: auth.user.id,
      actorRole: auth.user.role,
      eventType: AUDIT_EVENTS.CASE_TEMPLATE_PUBLISHED,
      entityType: 'CaseTemplate',
      entityId: caseTemplateId,
      summary: 'Case template published',
      details: {},
